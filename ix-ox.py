# van két játékos
# kijelző törlés
# a tábla megrajzolása
# jelölőket le kell rakni
# nyertes ellenőrzés
# ellenőrizni kell, hogy tele van-e tábla
import os
...
def clear_screen(): #megnézzük win, vagy mac, vagy linux
    command = "clear"
...
def input_player():
    ...
    while not (marker == "X" or marker == "O"):
        marker = input(PLAYER1_NAME + " Mivel szeretnél játszani X vagy O: ").upper()
    if marker == "X":
        return ("X", "O")
    return ("O", "X")
...
def check_winning(table, marker):
    ...
    (table[8] == marker and table[5] == marker and table[2] == marker) or #kozep oszlop
    (table[9] == marker and table[6] == marker and table[3] == marker) or #jobb oszlop
    (table[7] == marker and table[5] == marker and table[3] == marker) or #kereszt1
    (table[9] == marker and table[5] == marker and table[1] == marker) #jobb kereszt2
    )
...
while True:
    ...
    start_game = input("Készen állsz a harcra? Igen vagy nem: ")
    ...
    repeat_game = (input("Újabb játék? Igen vagy nem: ").lower().startswith("i"))
    if not repeat_game:
        break
