...
def fake_io_task(task_id: int, delay: float = 1.0, where: str = "SZINKRON"):
    print(f"[{where}] Task {task_id} START (thread: {threading.current_thread().name})")
    time.sleep(delay)
    print(f"[{where}] Task {task_id} END (thread: {threading.current_thread().name})")
...
def run_sync_io(num_task: int = 10):
    ...
    print(f"[SZINKRON] össz futtaási idő: {total:.2f} mp")
...
async def run_async_io(num_task: int = 10):
    ...
    total = time.time() - start
    print(f"[ASSZINKRON] össz futtaási idő: {total:.2f} mp")
...
def run_threaded_io(num_task: int = 10):
    ...
    for i in range(1, num_task + 1):
        t = threading.Thread(target=fake_io_task, args=(i,1.0, "THREADED"))
        threads.append(t)
        t.start()
    ...
    total = time.time() - start
    print(f"[THREADES] össz futtaási idő: {total:.2f} mp")
